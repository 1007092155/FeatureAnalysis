...
from findSimilar import eva_regress
from findSimilar import plot_results
...
class Distribution:
    ...
    up_train = np.array(up_data.rolling(window=6).mean())
    down_data = pd.DataFrame(down[attrs].values)
    down_train = np.array(down_data.rolling(window=6).mean())
    left_data = pd.DataFrame(left[attrs].values)
    left_train = np.array(left_data.rolling(window=6).mean())
    right_data = pd.DataFrame(right[attrs].values)
    right_train = np.array(right_data.rolling(window=6).mean())

    x_scaler = MinMaxScaler(feature_range=(0, 1)).fit(right_train[5:8352, :])
    x_train = x_scaler.transform(right_train[5:8352, :]).reshape(1, -1)[0]
    x_train = series_to_supervised(pd.DataFrame(x_train), 6, 1)
    del x_train['var1(t)']

    y_scaler = MinMaxScaler(feature_range=(0, 1)).fit(up_train[11:8352, :])
    y_train = y_scaler.transform(up_train[11:8352, :]).reshape(-1, 1).ravel()
    x_train['var1(t)'] = y_train
    train = np.array(x_train)

    x_test = x_scaler.transform(right_train[8346:, :]).reshape(1, -1)[0]
    x_test = series_to_supervised(pd.DataFrame(x_test), 6, 1)
    del x_test['var1(t)']
    # print(x_test.shape)
    y_test = up_train[8352:, :].reshape(1, -1)[0]
    # print(y_test)
    test = np.array(x_test)

    pred = []
    for test_data in test:
        train_data = DWTSimilar(test_data, train, 1000)
        np.random.shuffle(train_data)
        x_train = train_data[:, :-1]
        y_train = train_data[:, -1]
        pred.append(svr_predict(x_train, y_train, test_data.reshape(1, -1)))

    predicted = y_scaler.inverse_transform(np.array(pred).reshape(-1, 1)).reshape(1, -1)[0]
